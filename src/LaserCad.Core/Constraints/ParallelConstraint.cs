...
/// <summary>
/// Constraint wymuszajacy rownoleglosc dwoch odcinkow.
/// MVP dopasowuje drugi odcinek do kierunku pierwszego.
/// </summary>
public sealed class ParallelConstraint : ISketchConstraint
{
    public ParallelConstraint(Guid referenceEntityId, Guid constrainedEntityId, Guid? id = null)
    {
    ...
    public SketchConstraintKind Kind => SketchConstraintKind.Parallel;
    ...
    public Sketch Apply(Sketch sketch)
    {
        ...
        var end = new Point2D(
            constrained.Segment.Start.X + (direction.X * constrained.Segment.Length),
            constrained.Segment.Start.Y + (direction.Y * constrained.Segment.Length));
